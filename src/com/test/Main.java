...
public class Main {
    ...
    public static void interfaceNotIf() {
        ...
        typeMap.put("notCompare", new SampleD());

        String str = typeMap.get(request)==null?
                     typeMap.get("notCompare").validate(data1, data2, data3):
                     typeMap.get(request).validate(data1, data2, data3);

        System.out.println(str);
    }

    //interface 를 이용한 클래스 분류.
    public static void tryInterfaceNotIf() {
        Map<String, SampleInterface> typeMap = new HashMap<>();
        ...
        String str = "";
        try {
            str = typeMap.get(request).validate(data1, data2, data3);
        } catch (NullPointerException npe) {
            str = typeMap.get("notCompare").validate(data1, data2, data3);
        }
